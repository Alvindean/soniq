/**
 * SONIQ — Flow: concept → song.
 * Single endpoint serving two modes to stay within the 12-function Hobby cap.
 *
 *   POST /api/flow { mode: 'spec', concept }
 *     → { spec: FlowSpec }
 *
 *   POST /api/flow { mode: 'song', concept, spec }
 *     → { title, lyrics, sunoPrompt }
 */
...
module.exports = async function handler(req, res) {
  ...
  const isPreview = /^https:\/\/project-47rl2-[a-z0-9\-]+\.vercel\.app$/i.test(origin);
...
Write the lyrics now.`;
  ...
  return res.status(400).json({ error: 'invalid mode (use "spec" or "song")' });
};
