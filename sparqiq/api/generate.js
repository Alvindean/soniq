...
async function callOpenRouter(apiKey, messages, system, max_tokens, model) {
  const orModel = model || 'anthropic/claude-sonnet-4-5';
  ...
  let r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    body: JSON.stringify({ model: orModel, max_tokens, messages: orMsgs })
  });
  ...
  if (r.status === 402) {
    ...
    if (affordable >= 300) {
      r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        body: JSON.stringify({ model: orModel, max_tokens: affordable, messages: orMsgs })
      });
    } else {
...
module.exports = async function handler(req, res) {
  ...
  const { messages, system, max_tokens = 3000, model } = body || {};
  ...
  if (anthropicKey) {
    try {
      ...
      console.error('Anthropic failed, trying OpenRouter:', err.message);
  ...
  if (openrouterKey) {
    try {
      const text = await callOpenRouter(openrouterKey, messages, system, max_tokens, model);
      return res.status(200).json({ content: [{ type: 'text', text }] });
    } catch (err) {
      if (err.message === 'CREDITS_LOW') {
        return res.status(402).json({ error: 'OpenRouter credit balance too low. Add credits at openrouter.ai/settings/credits' });
      }
